...
static void draw_status_bar(lv_obj_t *screen) {
  ...
  rect(screen, 137, 0, 3, 45, 0xFFFFFF);
  rect(screen, 207, 0, 3, 45, 0xFFFFFF);
  rect(screen, 290, 0, 3, 45, 0xFFFFFF);
  rect(screen, 392, 0, 3, 45, 0xFFFFFF);
...
void math_menu_create(void) {
  ...
  lv_obj_t *graphing = image(screen, &ui_math_graphing_calc, 100, 115, true);
  lv_obj_t *scientific = image(screen, &ui_math_scientific_calculator, 250, 115, true);
  lv_obj_add_event_cb(graphing, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_GRAPHING_CALCULATOR);
  lv_obj_add_event_cb(scientific, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_SCIENTIFIC_CALCULATOR);
  label(screen, "Graphing\nCalculator", &monogram_24, 80, 265, 180, 52);
  label(screen, "Scientific\nCalculator", &monogram_24, 230, 265, 180, 52);
  button_input_register_active_app(screen, UI_STATE_MATH_MENU, math_menu_destroy);
  lv_screen_load(screen);
}

void math_menu_destroy(void) {
}
