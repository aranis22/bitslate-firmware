...
static lv_obj_t *card(lv_obj_t *parent, const lv_image_dsc_t *source, int x, int y) {
...
static void draw_status_bar(lv_obj_t *screen) {
  ...
  card(screen, &ui_home_capybara, 0, -1);
  label(screen, "test", &monogram_32, 53, 13, 80, 26);
  card(screen, &ui_home_mail, 145, 3);
  card(screen, &ui_home_battery, 400, 6);
...
void physics_menu_create(void) {
  ...
  card(screen, &ui_home_background_1, 0, 0);
  draw_status_bar(screen);

  lv_obj_t *mass = card(screen, &ui_physics_mass_springs, LEFT_COL_X, TOP_ROW_Y);
  lv_obj_t *collision = card(screen, &ui_physics_collision, MIDDLE_COL_X, TOP_ROW_Y);
  lv_obj_t *circuit = card(screen, &ui_physics_circuit, RIGHT_COL_X, TOP_ROW_Y);
  lv_obj_t *waves = card(screen, &ui_physics_waves, LEFT_COL_X, BOTTOM_ROW_Y);
  lv_obj_t *kinematics = card(screen, &ui_physics_kinematics, MIDDLE_COL_X, BOTTOM_ROW_Y);
  lv_obj_t *forces = card(screen, &ui_physics_forces, RIGHT_COL_X, BOTTOM_ROW_Y);
  lv_obj_add_event_cb(mass, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_MASSES_SPRINGS);
  lv_obj_add_event_cb(collision, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_COLLISION);
  lv_obj_add_event_cb(circuit, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_CIRCUITS);
  lv_obj_add_event_cb(waves, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_NONE);
  lv_obj_add_event_cb(kinematics, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_NONE);
  lv_obj_add_event_cb(forces, app_card_clicked, LV_EVENT_CLICKED, (void *)(intptr_t)APP_ID_NONE);

  label(screen, "masses/\nsprings", &monogram_24, LEFT_COL_X - 8, 151, CARD_W + 16, 42);
  label(screen, "Collision", &monogram_24, MIDDLE_COL_X - 10, 158, CARD_W + 20, 26);
  label(screen, "Circuits", &monogram_24, RIGHT_COL_X - 10, 158, CARD_W + 20, 26);
  label(screen, "Waves", &monogram_24, LEFT_COL_X - 10, 290, CARD_W + 20, 26);
  label(screen, "Kinematics", &monogram_24, MIDDLE_COL_X - 15, 290, CARD_W + 30, 26);
  label(screen, "Forces", &monogram_24, RIGHT_COL_X - 10, 290, CARD_W + 20, 26);

  button_input_register_active_app(screen, UI_STATE_PHYSICS_MENU, physics_menu_destroy);
  lv_screen_load(screen);
}

void physics_menu_destroy(void) {
}
