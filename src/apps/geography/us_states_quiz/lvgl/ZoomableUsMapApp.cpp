...
#include "../core/ZoomableUsMapCore.h"
...
constexpr int MAP_W = 390;
constexpr int MAP_H = 250;
constexpr int MAP_BYTES = MAP_W * MAP_H * 2;
constexpr int MAP_X = (SCREEN_W - MAP_W) / 2;
constexpr int MAP_Y = 40;
constexpr int BUTTON_Y = 294;
constexpr int BUTTON_H = 24;
constexpr int SMALL_BUTTON_W = 42;
constexpr int RESET_BUTTON_W = 82;
constexpr int GRID_BUTTON_W = 70;
constexpr int LABELS_BUTTON_W = 78;
constexpr int DRAG_TAP_THRESHOLD = 7;
constexpr int CAPITAL_DOT_SIZE = 7;
constexpr int CAPITAL_LABEL_W = 76;
constexpr int CAPITAL_LABEL_H = 14;
constexpr int STATE_LABEL_W = 92;
constexpr int STATE_LABEL_H = 14;

bitslate::geo::ZoomableUsMapCore mapCore;
bitslate::geo::ZoomableMapRenderOptions renderOptions{true, true};
...
lv_obj_t* capitalDots[16] = {};
lv_obj_t* capitalLabels[16] = {};
lv_obj_t* capitalLabelShadows[16] = {};
lv_obj_t* stateLabels[48] = {};
lv_obj_t* stateLabelShadows[48] = {};
...
const char* stateName(uint8_t stateId) {
  if (stateId >= bitslate::geo::US_STATE_COUNT) {
    return nullptr;
  }
  for (uint8_t i = 0; i < bitslate::geo::US_STATE_COUNT; ++i) {
    if (bitslate::geo::US_STATES[i].id == stateId) {
      return bitslate::geo::US_STATES[i].name;
    }
  }
  return nullptr;
}
...
void positionShadowedLabel(
    lv_obj_t* mainLabel,
    lv_obj_t* shadowLabel,
    int x,
    int y,
    bool visible
) {
  hideObj(mainLabel, !visible);
  hideObj(shadowLabel, !visible);
  if (visible && mainLabel != nullptr) {
    if (shadowLabel != nullptr) {
      lv_obj_set_pos(shadowLabel, x + 1, y + 1);
    }
...
void positionOverlays() {
  const bitslate::geo::CapitalMarker* capitals = mapCore.capitalMarkers();
  const uint8_t capitalCount = mapCore.capitalMarkerCount();
  for (uint8_t i = 0; i < capitalCount; ++i) {
    float sx = 0.0f;
    float sy = 0.0f;
    ...
    const bool showCapitalLabel =
        labelsEnabled && mapCore.zoom() >= bitslate::geo::ZoomableUsMapCore::kCapitalLabelZoom && visible;
    hideObj(capitalLabels[i], !showCapitalLabel);
    positionShadowedLabel(
        capitalLabels[i],
        capitalLabelShadows[i],
        static_cast<int>(sx) - CAPITAL_LABEL_W / 2,
        static_cast<int>(sy) - CAPITAL_DOT_SIZE / 2 - CAPITAL_LABEL_H - 4,
        showCapitalLabel
    );
  }

  const bitslate::geo::StateLabelPoint* labels = mapCore.stateLabelPoints();
  const uint8_t labelCount = mapCore.stateLabelPointCount();
  for (uint8_t i = 0; i < labelCount; ++i) {
    float sx = 0.0f;
    float sy = 0.0f;
    mapCore.gridToScreen(labels[i].gridX, labels[i].gridY, MAP_W, MAP_H, sx, sy);
    const bool showStateLabel =
        labelsEnabled && mapCore.zoom() >= bitslate::geo::ZoomableUsMapCore::kStateLabelZoom &&
        screenPointVisible(sx, sy, 24);
    hideObj(stateLabels[i], !showStateLabel);
    positionShadowedLabel(
        stateLabels[i],
        stateLabelShadows[i],
        static_cast<int>(sx) - STATE_LABEL_W / 2,
        static_cast<int>(sy) - STATE_LABEL_H / 2,
        showStateLabel
    );
  }
}
...
void handleMapTap(const lv_point_t& point) {
  ...
  if (!pointInMap(point, localX, localY)) {
    setStatus("Outside map");
    return;
  }

  const uint8_t stateId = mapCore.stateAtScreen(
      static_cast<float>(localX),
      static_cast<float>(localY),
      MAP_W,
      MAP_H
  );
  const char* name = stateName(stateId);
  if (name == nullptr) {
    setStatus("Outside map");
    return;
  }
...
void createOverlays() {
  ...
  for (uint8_t i = 0; i < capitalCount; ++i) {
    ...
    styleBox(capitalDots[i], 0x101418, 0xF8FAFC, 2, CAPITAL_DOT_SIZE / 2);
    lv_obj_set_size(capitalDots[i], CAPITAL_DOT_SIZE, CAPITAL_DOT_SIZE);

    capitalLabelShadows[i] = makeLabel(mapArea, capitals[i].capitalName, &monogram_16, 0x000000);
    lv_obj_set_size(capitalLabelShadows[i], CAPITAL_LABEL_W, CAPITAL_LABEL_H);
    styleOverlayLabel(capitalLabelShadows[i], 0x000000);

    capitalLabels[i] = makeLabel(mapArea, capitals[i].capitalName, &monogram_16, 0x101418);
  ...
  const uint8_t labelCount = mapCore.stateLabelPointCount();
  for (uint8_t i = 0; i < labelCount; ++i) {
    stateLabelShadows[i] = makeLabel(mapArea, labels[i].stateName, &monogram_16, 0x000000);
    lv_obj_set_size(stateLabelShadows[i], STATE_LABEL_W, STATE_LABEL_H);
    styleOverlayLabel(stateLabelShadows[i], 0x000000);

    stateLabels[i] = makeLabel(mapArea, labels[i].stateName, &monogram_16, 0x101418);
    lv_obj_set_size(stateLabels[i], STATE_LABEL_W, STATE_LABEL_H);
    styleOverlayLabel(stateLabels[i], 0xFFFFFF);
  }
}

void createMap(lv_obj_t* screen) {
  mapArea = lv_obj_create(screen);
  styleBox(mapArea, 0xE8EEF2, 0xCBD5E1, 1, 2);
  ...
  if (mapPixels == nullptr) {
    ...
    Serial.printf("Zoomable USA map buffer allocation failed (%d bytes)\n", MAP_BYTES);
    return;
  }

  Serial.printf("Zoomable USA map %s (%d bytes)\n", bufferStatus, MAP_BYTES);
...
void ZoomableUsMapApp::create() {
  ...
  titleLabel = makeLabel(screen, "Zoomable USA Map", &monogram_24, 0x172033);
  lv_obj_set_pos(titleLabel, 10, 4);

  statusLabel = makeLabel(screen, "Drag, tap, or zoom", &monogram_20, 0x334155);
  lv_obj_set_pos(statusLabel, 10, 24);
  lv_obj_set_width(statusLabel, SCREEN_W - 20);
