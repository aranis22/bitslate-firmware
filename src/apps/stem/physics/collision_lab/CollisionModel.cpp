...
void CollisionModel::setMass(int ballId, float mass) {
  CollisionBall& ball = mutableBall(ballId);
...
const CollisionBall& CollisionModel::getBall(int ballId) const {
...
CollisionBall& CollisionModel::mutableBall(int ballId) {
  return balls[ballId == 2 ? 1 : 0];
}
...
void CollisionModel::handleBallCollision() {
  CollisionBall* left = &balls[0];
  CollisionBall* right = &balls[1];
  if (left->x > right->x) {
    left = &balls[1];
    right = &balls[0];
  }
...
void CollisionModel::separatePair(CollisionBall& left, CollisionBall& right) {
  const float minDistance = left.radius + right.radius;
