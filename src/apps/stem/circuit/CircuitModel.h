...
public:
  ...
  const Component* getComponentAt(int index) const;
  const Wire* getWireAt(int index) const;
  int getTerminalId(int componentId, const char* name) const;
  bool getTerminalWorldPos(int terminalId, int* x, int* y) const;
  int findNearestTerminal(int x, int y, int maxDistance) const;
...
private:
  ...
  int distanceSquaredToTerminal(int terminalId, int x, int y) const;
};
