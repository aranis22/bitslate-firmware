...
class CircuitModel:
    ...
    def add_component(self, component_type: str, x: int, y: int) -> Component:
        component_id = next(self._component_ids)
        component = Component(component_id, component_type, x, y)
    ...
    def find_nearest_terminal(self, x: int, y: int, max_distance: int = SNAP_DISTANCE) -> Terminal | None:
        ...
        for component in self.components:
            for terminal in component.terminals:
                terminal_x, terminal_y = self.get_terminal_world_pos(terminal.id)
    ...
    def lit_bulb_ids(self) -> set[int]:
        lit: set[int] = set()
        batteries = [component for component in self.components if component.type == "battery"]
        bulbs = [component for component in self.components if component.type == "bulb"]
    ...
    def _build_graph(self) -> dict[int, list[tuple[int, int | None]]]:
        ...
        for component in self.components:
            if component.type == "bulb" and len(component.terminals) == 2:
